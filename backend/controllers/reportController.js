  ...
  try {
    ...
    const transactions = await db.query(
      ...
      SELECT 
        ...
        t.cash,
        t.checks,
        t.atm_mobile as atmMobile,
        t.paypal,
        t.total,
        t.date,
        t.status,
        t.created_at as createdAt,
        -- Added hours_worked field as hoursWorked
        t.hours_worked as hoursWorked,
        JSON_ARRAYAGG(
          JSON_OBJECT(
            'id', tb.book_id,
            'title', b.title,
...
export const getMonthlyReport = async (req, res) => {
  try {
    ...
    const today = new Date()
    const reportMonth = month ? Number.parseInt(month) : today.getMonth() + 1
    const reportYear = year ? Number.parseInt(year) : today.getFullYear()
...
export const getColporterReport = async (req, res) => {
  try {
    ...
    const financialConfig = await db.getOne("SELECT * FROM program_financial_config WHERE program_id = ?", [
      colporter.program_id,
    ])

    const transactions = await db.query(
      `SELECT t.*, t.transaction_date as date, t.hours_worked as hoursWorked,
       CONCAT(lp.first_name, ' ', lp.last_name) as leader_name
       FROM transactions t
       JOIN people lp ON t.leader_id = lp.id
       WHERE t.student_id = ? AND t.transaction_date BETWEEN ? AND ? AND t.status = 'APPROVED'
       ORDER BY t.transaction_date, t.created_at DESC`,
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      transactions[i].books = books || [] // Ensure books is always an array
    }
    ...
    const bibleStudies = await db.query(
      `SELECT bs.id, bs.name, bs.phone, bs.address, bs.location, bs.municipality_id,
       ...
       WHERE bs.colporter_id = ? AND bs.created_at BETWEEN ? AND ?
    ...
    const colporterPercentage = financialConfig?.colporter_percentage
      ? Number.parseFloat(financialConfig.colporter_percentage)
      : 50
    ...
    const charges = await db.query(
      `SELECT c.id, c.amount, c.reason, c.description, c.category, c.status, c.charge_date as date
       FROM charges c
       WHERE c.person_id = ? AND c.charge_date BETWEEN ? AND ? AND c.status = 'APPLIED'
    ...
    const advances = await db.query(
      `SELECT ca.id, ca.advance_amount as amount, ca.week_start_date, ca.week_end_date, ca.status, ca.request_date as date
       ...
       WHERE ca.person_id = ? AND ca.week_start_date BETWEEN ? AND ? AND ca.status = 'APPROVED'
...
export const getLeaderReport = async (req, res) => {
  try {
    ...
    const transactions = await db.query(
      `SELECT t.*, t.hours_worked as hoursWorked,
       CONCAT(sp.first_name, ' ', sp.last_name) as student_name
       FROM transactions t
       JOIN people sp ON t.student_id = sp.id
       WHERE t.leader_id = ? AND t.transaction_date BETWEEN ? AND ? AND t.status = 'APPROVED'
       ORDER BY t.transaction_date, t.created_at DESC`,
      [id, reportStartDate, reportEndDate],
    )
    ...
    const leaderPercentage = individualPercentage
      ? Number.parseFloat(individualPercentage.percentage)
      : financialConfig?.leader_percentage
        ? Number.parseFloat(financialConfig.leader_percentage)
        : 15
...
export const getProgramReport = async (req, res) => {
  try {
    ...
    const program = await db.getOne("SELECT * FROM programs WHERE is_active = TRUE")

    if (!program) {
      return res.status(404).json({ message: "No active program found" })
    }

    const reportStartDate = startDate || program.start_date
    const reportEndDate = endDate || new Date().toISOString().split("T")[0]

    const financialConfig = await db.getOne("SELECT * FROM program_financial_config WHERE program_id = ?", [program.id])
    ...
    const advances = await db.query(
      `SELECT ca.*, 
       ...
       WHERE ca.week_start_date BETWEEN ? AND ? AND ca.status = 'APPROVED'
       ORDER BY ca.week_start_date DESC`,
      [reportStartDate, reportEndDate],
    )

    const advanceTotals = advances.reduce(
      (acc, a) => {
    ...
    const leaderPerformance = await db.query(
      `SELECT 
       ...
       WHERE p.person_type = 'LEADER' AND t.transaction_date BETWEEN ? AND ? AND t.status = 'APPROVED'
       GROUP BY p.id
       ORDER BY total_sales DESC`,
      [reportStartDate, reportEndDate],
    )
    ...
    const colporterPerformance = await db.query(
      `SELECT 
       ...
       JOIN transactions t ON p.id = t.student_id
       JOIN people lp ON t.leader_id = lp.id
       WHERE p.person_type = 'COLPORTER' AND t.transaction_date BETWEEN ? AND ? AND t.status = 'APPROVED'
       GROUP BY p.id, lp.id
       ORDER BY total_sales DESC`,
      [reportStartDate, reportEndDate],
    )
    ...
    const goalProgress = {
      goal: Number.parseFloat(program.financial_goal),
      achieved: totals.total,
      remaining: Number.parseFloat(program.financial_goal) - totals.total,
      percentage: (totals.total / Number.parseFloat(program.financial_goal)) * 100,
    }
...
export const getIndividualEarningsReport = async (req, res) => {
  try {
    ...
    if (!startDate || !endDate) {
      ...
      const lastDay = new Date(today.getFullYear() + 1, 0)
    ...
    let transactionsQuery = `SELECT t.*, CONCAT(lp.first_name, ' ', lp.last_name) as leader_name FROM transactions t JOIN people lp ON t.leader_id = lp.id WHERE t.student_id = ? AND t.transaction_date BETWEEN ? AND ? AND t.status = 'APPROVED' ORDER BY t.transaction_date, t.created_at DESC`
    let transactionParams = [person.id, reportStartDate, reportEndDate]
    console.log(person.id, reportStartDate, reportEndDate)
    if (person.person_type === "LEADER") {
      transactionsQuery = `SELECT t.*, CONCAT(sp.first_name, ' ', sp.last_name) as student_name FROM transactions t JOIN people sp ON t.student_id = sp.id WHERE t.leader_id = ? AND t.transaction_date BETWEEN ? AND ? AND t.status = 'APPROVED' ORDER BY t.transaction_date, t.created_at DESC`
    ...
    for (let i = 0; i < formattedTransactions.length; i++) {
      const books = await db.query(
        `SELECT tb.book_id as id, b.title, b.size, tb.quantity, tb.price FROM transaction_books tb JOIN books b ON tb.book_id = b.id WHERE tb.transaction_id = ?`,
    ...
    if (person.person_type === "COLPORTER") {
      percentage = financialConfig?.colporter_percentage ? Number.parseFloat(financialConfig.colporter_percentage) : 50
    } else {
      const individualPercentage = await db.getOne(
        "SELECT percentage FROM leader_percentages WHERE leader_id = ? AND program_id = ? AND is_active = TRUE",
    ...
    const charges = await db.query(
      `SELECT c.id, c.amount, c.reason, c.description, c.category, c.status, c.charge_date as date FROM charges c WHERE c.person_id = ? AND c.charge_date BETWEEN ? AND ? AND c.status = 'APPLIED' ORDER BY c.charge_date DESC`,
    ...
    const advances = await db.query(
      `SELECT ca.id, ca.advance_amount as amount, ca.week_start_date, ca.week_end_date, ca.status, ca.request_date as date FROM cash_advances ca WHERE ca.person_id = ? AND ca.week_start_date BETWEEN ? AND ? AND ca.status = 'APPROVED' ORDER BY ca.week_start_date DESC`,
      [person.id, reportStartDate, reportEndDate],
    )
    ...
    if (person.person_type === "LEADER") {
      expenses = await db.query(
        `SELECT e.id, e.amount, e.motivo, e.category, e.notes, e.expense_date as date, e.created_at as createdAt, e.updated_at as updatedAt FROM expenses e WHERE e.leader_id = ? AND e.expense_date BETWEEN ? AND ? AND e.status = 'APPROVED' ORDER BY e.expense_date DESC`,
        [person.id, reportStartDate, reportEndDate],
      )
    ...
    formattedTransactions.forEach((transaction) => {
      ...
      const hoursWorked = Number(transaction.hours_worked) || 0
    ...
    if (dailySalesEntries.length > 0) {
      bestDay = dailySalesEntries.reduce((best, [date, amount]) => (amount > best.amount ? { date, amount } : best), {
        date: "",
        amount: 0,
      })
      ...
      if (worstDay.amount === Number.POSITIVE_INFINITY) {
...
export const getSalesHistory = async (req, res) => {
  try {
    ...
    if (user.person_id) {
      const person = await db.getOne("SELECT id, person_type, program_id FROM people WHERE id = ?", [user.person_id])
    ...
    switch (period) {
      ...
      case "all":
      default:
        const program = await db.getOne("SELECT start_date FROM programs WHERE id = ?", [programId])
    ...
    if (user.role === "ADMIN") {
      salesData = await db.query(
        `SELECT transaction_date as date, COALESCE(SUM(total), 0) as amount FROM transactions WHERE transaction_date BETWEEN ? AND ? AND status = 'APPROVED' GROUP BY transaction_date ORDER BY transaction_date`,
        [startDateStr, endDateStr],
      )
    } else if (personType === "LEADER") {
      salesData = await db.query(
        `SELECT transaction_date as date, COALESCE(SUM(total), 0) as amount FROM transactions WHERE leader_id = ? AND transaction_date BETWEEN ? AND ? AND status = 'APPROVED' GROUP BY transaction_date ORDER BY transaction_date`,
        [personId, startDateStr, endDateStr],
      )
    } else if (personType === "COLPORTER") {
      salesData = await db.query(
        `SELECT transaction_date as date, COALESCE(SUM(total), 0) as amount FROM transactions WHERE student_id = ? AND transaction_date BETWEEN ? AND ? AND status = 'APPROVED' GROUP BY transaction_date ORDER BY transaction_date`,
        [personId, startDateStr, endDateStr],
      )
    } else {
      salesData = []
...
export const getFinancialSummary = async (req, res) => {
  try {
    ...
    const program = await db.getOne("SELECT * FROM programs WHERE id = ?", [programId])
    ...
    if (user.role === "ADMIN") {
      const salesData = await db.getOne(
        `SELECT COALESCE(SUM(total), 0) as total_sales, (SELECT COALESCE(SUM(total), 0) FROM transactions WHERE transaction_date = ? AND status = 'APPROVED') as daily_sales, (SELECT COALESCE(SUM(total), 0) FROM transactions WHERE transaction_date BETWEEN ? AND ? AND status = 'APPROVED') as weekly_sales, (SELECT COALESCE(SUM(total), 0) FROM transactions WHERE transaction_date BETWEEN ? AND ? AND status = 'APPROVED') as monthly_sales FROM transactions WHERE transaction_date BETWEEN ? AND ? AND status = 'APPROVED'`,
      ...
      const salesData = await db.getOne(
        `SELECT COALESCE(SUM(total), 0) as total_sales, (SELECT COALESCE(SUM(total), 0) FROM transactions WHERE leader_id = ? AND transaction_date = ? AND status = 'APPROVED') as daily_sales, (SELECT COALESCE(SUM(total), 0) FROM transactions WHERE leader_id = ? AND transaction_date BETWEEN ? AND ? AND status = 'APPROVED') as weekly_sales, (SELECT COALESCE(SUM(total), 0) FROM transactions WHERE leader_id = ? AND transaction_date BETWEEN ? AND ? AND status = 'APPROVED') as monthly_sales FROM transactions WHERE leader_id = ? AND transaction_date BETWEEN ? AND ? AND status = 'APPROVED'`,
        [
          personId,
          todayStr,
          personId,
      ...
      const salesData = await db.getOne(
        `SELECT COALESCE(SUM(total), 0) as total_sales, (SELECT COALESCE(SUM(total), 0) FROM transactions WHERE student_id = ? AND transaction_date = ? AND status = 'APPROVED') as daily_sales, (SELECT COALESCE(SUM(total), 0) FROM transactions WHERE student_id = ? AND transaction_date BETWEEN ? AND ? AND status = 'APPROVED') as weekly_sales, (SELECT COALESCE(SUM(total), 0) FROM transactions WHERE student_id = ? AND transaction_date BETWEEN ? AND ? AND status = 'APPROVED') as monthly_sales FROM transactions WHERE student_id = ? AND transaction_date BETWEEN ? AND ? AND status = 'APPROVED'`,
        [
          personId,
          todayStr,
          personId,
