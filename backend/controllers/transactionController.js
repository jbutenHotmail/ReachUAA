...
export const getTransactions = async (req, res) => {
  try {
    ...
    let query = `
      ...
      t.hours_worked as hoursWorked, t.created_at as createdAt, t.updated_at as updatedAt
      FROM transactions t
...
export const createTransaction = async (req, res) => {
  try {
    const { studentId, leaderId, cash, checks, atmMobile, paypal, date, books, programId, hoursWorked } = req.body
    ...
    const result = await db.transaction(async (connection) => {
      ...
      const [transactionResult] = await connection.execute(
        "INSERT INTO transactions (student_id, leader_id, cash, checks, atm_mobile, paypal, total, transaction_date, status, created_by, program_id, hours_worked) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          studentId,
          leaderId,
          cash || 0,
...
export const updateTransaction = async (req, res) => {
  try {
    const { id } = req.params
    const { studentId, leaderId, cash, checks, atmMobile, paypal, date, status, books, programId, hoursWorked } =
      req.body

    // Check if transaction exists
    const existingTransaction = await db.getOne("SELECT * FROM transactions WHERE id = ?", [id])
    ...
    const newChecks = checks !== undefined ? checks : existingTransaction.checks
    const newAtmMobile = atmMobile !== undefined ? atmMobile : existingTransaction.atm_mobile
    const newPaypal = paypal !== undefined ? paypal : existingTransaction.paypal
    ...
    await db.transaction(async (connection) => {
      // Update transaction
      await connection.execute(
        "UPDATE transactions SET student_id = ?, leader_id = ?, cash = ?, checks = ?, atm_mobile = ?, paypal = ?, total = ?, transaction_date = ?, status = ?, program_id = ?, hours_worked = ?, updated_at = NOW() WHERE id = ?",
        [
          ...
          hoursWorked !== undefined ? hoursWorked : existingTransaction.hours_worked,
          id,
        ],
      )
      ...
      if (books) {
        ...
        await connection.execute("DELETE FROM transaction_books WHERE transaction_id = ?", [id])
...
export const approveTransaction = async (req, res) => {
  try {
    ...
    await db.transaction(async (connection) => {
      ...
      await connection.execute("UPDATE transactions SET status = ?, updated_at = NOW() WHERE id = ?", ["APPROVED", id])
      ...
      for (const book of books) {
        ...
        if (programBooks.length > 0) {
          ...
          const [bookInfo] = await connection.execute("SELECT price FROM books WHERE id = ?", [book.book_id])
...
export const rejectTransaction = async (req, res) => {
  try {
    ...
    if (existingTransaction.status === "REJECTED") {
      return res.status(400).json({ message: "Transaction is already rejected" })
    }
    ...
    if (existingTransaction.status === "APPROVED") {
      await db.transaction(async (connection) => {
        // Update transaction status
        await connection.execute("UPDATE transactions SET status = ?, updated_at = NOW() WHERE id = ?", [
          "REJECTED",
          id,
        ])
      ...
      // Just update status
      await db.update("UPDATE transactions SET status = ?, updated_at = NOW() WHERE id = ?", ["REJECTED", id])
    }
