  ...
  if (!transaction) {
    return (
      <div className="text-center py-12">
        ...
        <Button variant="outline" className="mt-4 bg-transparent" onClick={() => navigate("/transactions")}>
          {t("common.back")}
        </Button>
      </div>
    )
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-gray-900">{t("transactionDetails.title")}</h1>
        </div>
        {transaction.status === "PENDING" && isAdmin && (
          <div className="flex gap-2">
            ...
            <Button variant="danger" onClick={() => handleStatusChange("REJECTED")} leftIcon={<XCircle size={16} />}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {transaction.hoursWorked ? `${transaction.hoursWorked}h` : t("common.notSpecified")}
                  </p>
                </div>
                <div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-green-50 rounded-lg">
                  ...
                  <p className="text-lg font-semibold text-green-700">${Number(transaction.cash).toFixed(2)}</p>
                </div>
                <div className="p-3 bg-blue-50 rounded-lg">
                  <p className="text-sm text-blue-600">{t("transactionDetails.checks")}</p>
                  <p className="text-lg font-semibold text-blue-700">${Number(transaction.checks).toFixed(2)}</p>
                </div>
                <div className="p-3 bg-purple-50 rounded-lg">
                  <p className="text-sm text-purple-600">{t("transactionDetails.atmMobile")}</p>
                  <p className="text-lg font-semibold text-purple-700">${Number(transaction.atmMobile).toFixed(2)}</p>
                </div>
                <div className="p-3 bg-indigo-50 rounded-lg">
                  <p className="text-sm text-indigo-600">{t("transactionDetails.paypal")}</p>
                  <p className="text-lg font-semibold text-indigo-700">${Number(transaction.paypal).toFixed(2)}</p>
                </div>
              </div>
              ...
              <div className="p-3 bg-success-50 border border-success-200 rounded-lg flex items-start gap-2 mb-4">
                ...
                <div>
                  <p className="text-sm font-medium text-success-700">
                    {t("transactionDetails.extraDonationDetected")}
                  </p>
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant={bookSize === BookSize.LARGE ? "primary" : "success"} size="sm">
                        {t(`inventory.${bookSize === BookSize.LARGE ? "large" : "small"}`)}
                      </Badge>
                    </div>
                  </div>
                  <Badge variant={bookSize === BookSize.LARGE ? "primary" : "success"}>{book.quantity}</Badge>
                </div>
              )
            })}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="p-3 bg-green-50 rounded-lg">
                  ...
                  <p className="text-lg font-semibold text-green-700">${bookTotals.value.toFixed(2)}</p>
                </div>
                <div className={`p-3 rounded-lg ${surplus >= 0 ? "bg-success-50" : "bg-danger-50"}`}>
                  <p className={`text-sm ${surplus >= 0 ? "text-success-600" : "text-danger-600"}`}>
                    {t("transactionDetails.difference")}
                  </p>
                ...
                <div className="p-3 bg-success-50 rounded-lg border border-success-200">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-success-600">{t("transactionDetails.valuesMatchMessage")}</p>
                    </div>
                  </div>
                </div>
              )}
