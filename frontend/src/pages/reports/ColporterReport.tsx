...
import type { Colporter } from "../../types"
...
const ColporterReport: React.FC = () => {
  ...
  useEffect(() => {
    const getColporterId = async () => {
      try {
        ...
        const colporter = people.find((p: any) => `${p.name} ${p.apellido}` === name || p.name === name)
  ...
  const processTransactionData = (colporterTransactions: any[]) => {
    ...
    const totalHours = validTransactions.reduce((sum, t) => Number(sum) + (Number(t.hoursWorked) || 0), 0)
    ...
    const mostRecentWeek = weeks.sort((a, b) => new Date(b.endDate).getTime() - new Date(a.endDate).getTime())[0]
    ...
    if (mostRecentWeek) {
      ...
      setColporterData({
        ...
        horas: {
          total: totalHours,
          promedio: totalHours / validTransactions.length,
        },
      })
    }
  }
  ...
  if (!colporterData || !weeklyData) {
    return (
      <div className="p-4 bg-warning-50 border border-warning-200 rounded-lg text-warning-700">
        ...
        <Button variant="outline" className="mt-4 bg-transparent" onClick={() => navigate("/reports/week/finances")}>
          {t("common.back")}
        </Button>
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate("/reports/week/finances")}>
          <ChevronLeft size={20} />
        </Button>
        <div>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-900">
                {t("colporterReport.net", { percentage: program?.financialConfig?.colporter_percentage || 50 })}
              </h3>
              <div className="space-y-2">
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  <span className="text-sm font-medium text-gray-600">{t("common.total")}</span>
            ...
            {colporterData.horas && (
              <>
                <div className="flex justify-between items-center p-3 bg-info-50 rounded-lg">
                  <span className="text-sm font-medium text-info-600">{t("dashboard.totalHours")}</span>
                  <Badge variant="info">{colporterData.horas.total}h</Badge>
                </div>
                <div className="flex justify-between items-center p-3 bg-info-50 rounded-lg">
                  <span className="text-sm font-medium text-info-600">{t("dashboard.averageHours")}</span>
                  <Badge variant="info">{colporterData.horas.promedio.toFixed(1)}h</Badge>
                </div>
              </>
            )}
          </div>
                ...
                <tbody className="divide-y divide-gray-200">
                  <tr className="bg-yellow-50">
                    ...
                    <td className="px-4 py-3 text-sm text-right font-medium whitespace-nowrap">
                      $
                      {Object.values(weeklyData.sales)
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  ...
                  {Array.from(
                    new Set(Object.values(weeklyData.books).flatMap((dayBooks: any) => Object.keys(dayBooks.large))),
                      ...
                      {Object.keys(weeklyData.books)
                        ...
                        .map((date) => (
                          <td key={date} className="px-4 py-3 text-sm text-center">
                            <Badge variant="primary">{weeklyData.books[date]?.large[bookTitle] || 0}</Badge>
                          </td>
                        ))}
                      <td className="px-4 py-3 text-sm text-center font-medium">
                        <Badge variant="primary">
                          {Object.values(weeklyData.books).reduce(
                            (sum: number, dayBooks: any) => sum + (dayBooks.large[bookTitle] || 0),
                            0,
                          )}
                        </Badge>
                      </td>
                  ...
                  {Array.from(
                    new Set(Object.values(weeklyData.books).flatMap((dayBooks: any) => Object.keys(dayBooks.small))),
                  ).map((bookTitle: string) => (
                    <tr key={`small-${bookTitle}`} className="bg-success-50">
                      ...
                      {Object.keys(weeklyData.books)
                        ...
                        .map((date) => (
                          <td key={date} className="px-4 py-3 text-sm text-center">
                            <Badge variant="success">{weeklyData.books[date]?.small[bookTitle] || 0}</Badge>
                          </td>
                        ))}
                      <td className="px-4 py-3 text-sm text-center font-medium">
                        <Badge variant="success">
                          {Object.values(weeklyData.books).reduce(
                            (sum: number, dayBooks: any) => sum + (dayBooks.small[bookTitle] || 0),
                            0,
                          )}
                        </Badge>
                      </td>
                  ...
                  <tr className="bg-gray-100 font-semibold">
                    ...
                    {Object.keys(weeklyData.books)
                      .sort()
                      .map((date) => {
                        const largeBooksCount = Object.values(weeklyData.books[date]?.large || {}).reduce(
                          (sum: number, qty: any) => sum + qty,
                          0,
                        )
                        const smallBooksCount = Object.values(weeklyData.books[date]?.small || {}).reduce(
                          (sum: number, qty: any) => sum + qty,
                          0,
                        )
                        return (
                          <td key={date} className="px-4 py-3 text-sm text-center">
                            <Badge variant="secondary">{largeBooksCount + smallBooksCount}</Badge>
                          </td>
                        )
                      })}
