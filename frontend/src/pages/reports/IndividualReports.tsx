...
const IndividualReports: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        const totalSales = allTransactions
          .filter(t => new Date(t.date) >= new Date(firstDay) && new Date(t.date) <= new Date(lastDay) && t.status === 'APPROVED')
    ...
    loadInitialData();
  }, [fetchUsers, fetchPeople, fetchCharges, fetchAdvances, fetchProgram, wereTransactionsFetched, transactions, getLeaders]);
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      if (personType === 'COLPORTER') {
        ...
        const allProgramTransactions = fetchedTransactions.filter(t => 
      ...
      const netAmount = personType === 'COLPORTER'
        ...
        ${
          reportData.personType === 'LEADER'
            ...
            <div class="leader-note">
              <p><strong>Note:</strong> Leader earnings are calculated as ${reportData.percentage}% of the <strong>total program sales</strong> during this period, divided by ${totalLeadersCount} active leaders.</p>
              <p>Total program sales for this period: $${reportData.totalEarnings.toFixed(2)}</p>
            </div>
          `
            : ''
        }
  ...
  const exportReportCSV = () => {
    ...
    if (reportData.personType === 'COLPORTER') {
      ...
      csvContent += "Program Sales\r\n";
      csvContent += `Total Program Sales,${reportData.totalEarnings.toFixed(2)}\r\n`;
      csvContent += `Leader Percentage (${reportData.percentage}% ÷ ${totalLeadersCount}),${((reportData.totalEarnings * (reportData.percentage / 100)) / totalLeadersCount).toFixed(2)}\r\n\r\n`;
    }
    ...
    csvContent += `${reportData.personType} Percentage (${reportData.percentage}%${reportData.personType === 'LEADER' ? ` ÷ ${totalLeadersCount}` : ''}),,${(
      reportData.personType === 'COLPORTER'
        ? reportData.totalEarnings * (reportData.percentage / 100)
        : (reportData.totalEarnings * (reportData.percentage / 100)) / totalLeadersCount
    ).toFixed(2)}\r\n`;
                    ...
                    <tbody>
                      ...
                      <tr className="bg-gray-100 font-semibold">
                        ...
                        <td className="px-4 py-3 text-sm text-center whitespace-nowrap bg-purple-100">
                          {(() => {
                            const advanceTotal = reportData.advances.reduce(
                              (sum, a) => sum + a.amount,
                              0
                            );
                            return advanceTotal > 0 ? `-${advanceTotal.toFixed(2)}` : '-';
                      ...
                      <tr className="bg-success-100 font-bold">
                        <td
                          colSpan={8}
                          className="px-4 py-3 text-sm font-bold text-success-800"
                        >
                          Net Amount ({reportData.percentage}% of sales${reportData.personType === 'LEADER' ? ` ÷ ${totalLeadersCount}` : ''} - charges - advances)
                        </td>
                        <td
                          colSpan={3}
                          className="px-4 py-3 text-center text-sm font-bold text-success-800"
                        >
                ...
                <div className="p-6">
                  <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-blue-800 font-medium">
                      <strong>Note:</strong> Leader earnings are calculated as {reportData.percentage}% of the <strong>total program sales</strong> divided by {totalLeadersCount} active leaders.
                    </p>
                    <p className="text-blue-800">Total program sales: ${reportData.totalEarnings.toFixed(2)}</p>
                  </div>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div className="p-4 bg-primary-50 rounded-lg border border-primary-200">
                      <h3 className="text-lg font-semibold text-primary-800 mb-3">Program Sales</h3>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-primary-700">Total Program Sales:</span>
                        <span className="text-xl font-bold text-primary-900">${reportData.totalEarnings.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between items-center mt-2">
                        <span className="text-sm font-medium text-primary-700">Leader Earnings ({reportData.percentage}% ÷ {totalLeadersCount}):</span>
                        <span className="text-xl font-bold text-primary-900">${((reportData.totalEarnings * (reportData.percentage / 100)) / totalLeadersCount).toFixed(2)}</span>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-lg font-bold text-red-600">-${reportData.charges.reduce((sum, c) => sum + c.amount, 0).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between items-center mt-2">
                        <span className="text-sm font-medium text-gray-700">Total Advances:</span>
                        <span className="text-lg font-bold text-purple-600">-${reportData.advances.reduce((sum, a) => sum + a.amount, 0).toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-success-50 rounded-lg border border-success-200 mb-6">
                    <div className="flex justify-between items-center">
                      <h3 className="text-lg font-semibold text-success-800">Net Amount</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {reportData.charges.length > 0 && (
                      <div>
                        <h3 className="text-lg font-semibold text-gray-800 mb-3">Charges</h3>
                        <div className="overflow-x-auto">
                          <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                              {reportData.charges.map(charge => (
                                <tr key={charge.id}>
                                  <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900">{new Date(charge.date).toLocaleDateString()}</td>
                                  <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900">{charge.reason}</td>
                                  <td className="px-3 py-2 whitespace-nowrap text-sm text-right text-red-600">${charge.amount.toFixed(2)}</td>
                                </tr>
                              ))}
                              <tr className="bg-gray-50">
                                <td colSpan={2} className="px-3 py-2 whitespace-nowrap text-sm font-bold text-gray-900">Total</td>
                                <td className="px-3 py-2 whitespace-nowrap text-sm text-right font-bold text-red-600">${reportData.charges.reduce((sum, c) => sum + c.amount, 0).toFixed(2)}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                    ...
                    {reportData.advances.length > 0 && (
                      <div>
                        <h3 className="text-lg font-semibold text-gray-800 mb-3">Cash Advances</h3>
                        <div className="overflow-x-auto">
                          <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                              <tr>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                              {reportData.advances.map(advance => (
                                <tr key={advance.id}>
                                  <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900">{new Date(advance.date).toLocaleDateString()}</td>
                                  <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900">{new Date(advance.weekStartDate).toLocaleDateString()} - {new Date(advance.weekEndDate).toLocaleDateString()}</td>
                                  <td className="px-3 py-2 whitespace-nowrap text-sm text-right text-purple-600">${advance.amount.toFixed(2)}</td>
                                </tr>
                              ))}
                              <tr className="bg-gray-50">
                                <td colSpan={2} className="px-3 py-2 whitespace-nowrap text-sm font-bold text-gray-900">Total</td>
                                <td className="px-3 py-2 whitespace-nowrap text-sm text-right font-bold text-purple-600">${reportData.advances.reduce((sum, a) => sum + a.amount, 0).toFixed(2)}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-3 bg-white rounded-lg shadow-sm">
                      <span className="text-sm font-medium text-gray-600">
                        {reportData.personType === 'LEADER' ? 'Total Program Sales' : 'Total Sales'}
                      </span>
                      <span className="text-lg font-bold text-gray-900">${reportData.totalEarnings.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-white rounded-lg shadow-sm">
                      <span className="text-sm font-medium text-gray-600">
                        {reportData.personType} Percentage ({reportData.percentage}%{reportData.personType === 'LEADER' ? ` ÷ ${totalLeadersCount}` : ''})
                      </span>
                    ...
                    {reportData.personType === 'LEADER' && (
                      <div className="p-3 bg-blue-50 rounded-lg shadow-sm">
                        <p className="text-sm text-blue-800">
                          <strong>Leader Earnings:</strong> Calculated as {reportData.percentage}% of total program sales divided by {totalLeadersCount} active leaders.
                        </p>
                        <p className="text-sm text-blue-800 mt-1">
                          Total program sales: ${reportData.totalEarnings.toFixed(2)}
                        </p>
                      </div>
                    )}
